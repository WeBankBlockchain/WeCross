...
deploy_fabric_xa_chaincodes() {
    # deploy from 8250
    LOG_INFO "Deploy Fabric xa chaincodes: payment.fabric.asset"
...
EOF
    ...
    try_times=90
    i=0
    echo -e "\033[32masset chaincode is instantiating ...\033[0m\c"
...
deploy_bcos_xa_contracts() {
    # deploy from 8250
    LOG_INFO "Deploy BCOS xa contract: payment.fabric.asset"
    cd ${ROOT}/WeCross-Console/
...
main() {
    ...
    echo -e "
      FISCO BCOS                    Fabric
 \033[32m(payment.bcos.asset)      (payment.fabric.asset)\033[0m
           |                          |
           |                          |
