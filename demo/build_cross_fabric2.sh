...
config_router_8251() {
    ...
    cd ${router_dir}
    # stubs
    bash add_chain.sh -t Fabric2.0 -n fabric2 -d conf/chains
    cp ${fabric_demo_dir}/certs/chains/fabric2/* conf/chains/fabric2/

    # fabric stub internal accounts
    bash add_account.sh -t Fabric2.0 -n fabric2_admin -d conf/accounts
    cp ${fabric_demo_dir}/certs/accounts/fabric2_admin/* conf/accounts/fabric2_admin/
    bash add_account.sh -t Fabric2.0 -n fabric2_admin_org1 -d conf/accounts
    cp ${fabric_demo_dir}/certs/accounts/fabric2_admin_org1/* conf/accounts/fabric2_admin_org1/
    bash add_account.sh -t Fabric2.0 -n fabric2_admin_org2 -d conf/accounts
    cp ${fabric_demo_dir}/certs/accounts/fabric2_admin_org2/* conf/accounts/fabric2_admin_org2/
    sed_i 's/Org1MSP/Org2MSP/g' conf/accounts/fabric2_admin_org2/account.toml
...
build_account_manager() {
    ...
    if [ -d ${name} ]; then
        ...
        if [ -e download_account_manager.sh ]; then
            ...
            bash <(curl -sL https://${GIT_URL_BASE}/WebankBlockchain/WeCross/releases/download/resources/download_account_manager.sh) -t "${WECROSS_ACCOUNT_MANAGER_VERSION}" -u ${DB_USERNAME} -p ${DB_PASSWORD} -H ${DB_IP} -P ${DB_PORT}
        fi
    fi
...
add_fabric_account() {
    ...
    bash start.sh <<EOF
login
addChainAccount Fabric2.0 conf/accounts/${name}/account.crt conf/accounts/${name}/account.key ${mspid} true
quit
EOF
    cd -
}
...
deploy_chain_account() {
    ...
    cp -r ${ROOT}/fabric2/certs/accounts/* ${ROOT}/WeCross-Console/conf/accounts/

    add_bcos_account bcos_user1                  # 0
    add_fabric_account fabric2_admin_org1 Org1MSP # 1
    add_fabric_account fabric2_admin_org2 Org2MSP # 2
    # add_fabric_account fabric2_user1 Org1MSP      # 3
...
deploy_sample_resource() {
    ...
    # fabric sample resource has been deployed by fabric2 demo
}
...
main() {
    ...
    # config routers
    config_router_8250 ${ROOT}/routers-payment/127.0.0.1-8250-25500/ ${ROOT}/fabric2 ${ROOT}/bcos
    config_router_8251 ${ROOT}/routers-payment/127.0.0.1-8251-25501/ ${ROOT}/fabric2
...
help() {
    echo "$1"
    cat <<EOF
Create a wecross demo with bcos and fabric2 chains.
Usage:
